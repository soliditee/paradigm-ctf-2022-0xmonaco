...
contract ExampleCar is Car {
    ...
    function takeYourTurn(Monaco.CarData[] calldata allCars, uint256 ourCarIndex) external override {
        ...
        // If this is the first time, buy a lot of accelerate
        uint16 currentTurn = monaco.turns();
        if (currentTurn <= 3) {
            if (ourCar.balance > monaco.getAccelerateCost(8)) {
        ...
        if (
            ...
            ourCar.balance -= uint24(monaco.buyShell(1)); // This will instantly set the car in front of us' speed to 0.
        }
    }
}
